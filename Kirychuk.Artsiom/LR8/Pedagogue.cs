...
namespace laba6
{
    ...
    delegate void PremHandler(string mass);
    ...
    class Pedagogue : Kindergarten
    {
        ...
        public event PremHandler NotifyAdditive;
        ...
        {
            NotifyAdditive?.Invoke($"Было зачисленно {premium}");
            double percent = (double) premium / (double)_salary * 100;
            if (percent < 5)
            {
                _spirits = Mood.Normal; ;
                Console.WriteLine("На телефон положу! ");                
            }
