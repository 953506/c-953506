...
namespace Laba8
{
    ...
    {
        public delegate void EntranceHandler(string message);
        public event EntranceHandler Notify;
        public Data data;
        public string _name;
        ...
        public void Begin()
        {
            Notify?.Invoke($"Начало сдачи цт\n");
        }

        public delegate void Abc();

        public static void GiveAbc(Abc mes)
        {
            mes?.Invoke();
        }
    ...
    class Program
    {
        ...
        {
            Entrance.GiveAbc(IBeg.Welcome);
            Random x = new Random();
            string name = "";
            int age = 18, levelMath = 1, levelRus = 1, levelPhys = 1;
            int[] nothing = { 1, 1, 1 };
            Entrance chel1 = new Entrance("", 18, nothing);
            Entrance chel2 = new Entrance("", 18, nothing);
            chel1.Notify += delegate (string message)
            {
            ...
            for (int i = 0; i < 2; i++)
            {
                try
                {
                    ...
                    levelMath = Convert.ToInt32(Console.ReadLine());
                    Console.Write("Уровень подготовки к русскому: ");
                    levelRus = Convert.ToInt32(Console.ReadLine());
                    Console.Write("Уровень подготовки к физике: ");
                    levelPhys = Convert.ToInt32(Console.ReadLine());
                }
                catch
                {
